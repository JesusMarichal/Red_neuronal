...
/// Los navegadores de esta instalación no siempre pueden acceder a
/// `mlbstatic.com` directamente. El panel entrega los escudos desde localhost
/// y los conserva en caché para no volver a descargarlos en cada actualización.
fn team_logo(id: &str) -> Result<Vec<u8>, String> {
    if id.is_empty() || !id.bytes().all(|b| b.is_ascii_digit()) {
        return Err("id de equipo inválido".into());
    }

    let path = Path::new("data/raw/logos").join(format!("{id}.svg"));
    if let Ok(bytes) = fs::read(&path) {
        return Ok(bytes);
    }

    let url = format!("https://www.mlbstatic.com/team-logos/{id}.svg");
    let mut response = ureq::get(&url)
        .call()
        .map_err(|e| format!("no se pudo descargar el logo {id}: {e}"))?;
    let bytes = response
        .body_mut()
        .with_config()
        .limit(2 * 1024 * 1024)
        .read_to_vec()
        .map_err(|e| format!("no se pudo leer el logo {id}: {e}"))?;
...
fn handle(stream: &mut TcpStream, state: &RwLock<Snapshot>) {
    ...
    {
        match team_logo(id) {
            Ok(bytes) => respond(stream, "200 OK", "image/svg+xml", &bytes),
            Err(e) => respond(stream, "404 Not Found", "text/plain; charset=utf-8", e.as_bytes()),
        }
        return;
    }
